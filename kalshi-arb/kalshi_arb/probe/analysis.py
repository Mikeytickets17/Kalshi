...
# Production probe acceptance thresholds. The CLI uses these to decide
# whether to write detected_limits.yaml and whether to emit PASS.
# Rationale:
#   - REST write probe needs >=80% success so p95 is meaningful
#   - WS subscription cap must have confirmed >=50 tickers
#   - Rate-limit probe may not hit 429 if we stay under the true
#     ceiling (that's ok; max_successful_rps>=10 still useful)
#   - E2E loop needs >=5 samples to produce a p95 with any signal
PROD_MIN_WRITE_SUCCESS_PCT = 80.0
PROD_MIN_WS_CONFIRMED_TICKERS = 50
PROD_MIN_RATE_RPS = 10
PROD_MIN_E2E_SAMPLES = 5
